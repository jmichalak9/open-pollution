...
resource "google_project_iam_binding" "open-pollution-project-owners" {
  role    = "roles/owner"
  project = local.project

  members = [
    "serviceAccount:terraform@${local.project}.iam.gserviceaccount.com",
...
resource "google_project_iam_binding" "open-pollution-project-editors" {
  role    = "roles/editor"
  project = local.project
